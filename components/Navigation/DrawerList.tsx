...
const DrawerList: FC<Props> = ({ onClose }) => {
	...
	const onClickHandler = useCallback(
		() => fold(ioVoid, onClose)(fromPredicate((m: boolean) => m)(matches)),
		[onClose, matches]
	);
				...
				.map((route) =>
					route.target ? (
						<li key={route.name}>
							...
							>
								<ListItemIcon>
									<Icon style={!matches ? { color: theme.palette.background.default } : {}}>
										{route.icon}
									</Icon>
								</ListItemIcon>
								<ListItemText
						...
						<li key={route.name}>
							<Link href={route.path} passHref>
								<ListItem button onClick={onClickHandler} className={classes.listItem}>
									<ListItemIcon>
										<Icon style={!matches ? { color: theme.palette.background.default } : {}}>
											{route.icon}
										</Icon>
